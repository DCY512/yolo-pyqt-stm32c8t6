    ...
    def open_frame(self):
        ...
        if ret:
            try:
                ...
                filtered_cls_list = []
                filtered_conf_list = []
                sendnum_list = []  # 发送的类别索引

                for i in range(len(conf_list)):
                    if cls_list[i] == 0:
                        continue  # 忽略类别为 0 的目标

                    # 置信度大于 0.7 才会被记录和发送
                    if conf_list[i] >= 0.7:
                        sendnum_list.append(int(cls_list[i]))

                    filtered_locations.append([int(x) for x in location_list[i]])
                    filtered_cls_list.append(int(cls_list[i]))
                ...
                if textsend:
                    self.send_data_over_serial(textsend)

                # 计算类别分布百分比
                total_nums = len(self.location_list)
                cls_percents = [self.cls_list.count(i) / total_nums if total_nums else 0 for i in range(6)]
                self.set_percent(cls_percents)

                # **重新绘制目标框（只绘制过滤后的目标）**
                now_img = now_img.copy()
                for i, box in enumerate(self.location_list):
                    color = tools.Colors()(self.cls_list[i], bgr=True)  # 颜色
                    now_img = tools.drawRectBox(now_img, box, Config.CH_names[self.cls_list[i]], self.fontC, color)
